...
import {
  ...
  BASE_MATRIX, PROPERTIES_DATA, PROPERTY_INSIGHTS, PACE_DISTRIBUTION
...
const Card = ({ children, style = {} }) => (
  <div style={{ background:'#fff', borderRadius:14, border:'1px solid #e2e8f0', padding:'20px 22px', ...style }}>
    {children}
  </div>
)
...
const InsightBar = ({ propName }) => {
  ...
  return (
    <div style={{ background:'linear-gradient(135deg,#0f172a 0%,#1e293b 100%)', borderRadius:12, padding:'16px 20px', marginBottom:20, border:'1px solid #334155' }}>
      <div style={{ fontSize:10, letterSpacing:2, color:'#475569', textTransform:'uppercase', fontFamily:"'DM Mono',monospace", marginBottom:12 }}>
        BOOKING PACE INSIGHT · 3년 실데이터 기반
      </div>
...
const RevPARInput = ({ value, onChange, label }) => {
  const isEmpty = !value || value === ''
  return (
    <div>
      <div style={{ fontSize:10, color:'#64748b', fontWeight:600, marginBottom:5, textTransform:'uppercase', letterSpacing:0.5 }}>{label}</div>
      <input type="number" value={value} placeholder="입력" onChange={e => onChange(e.target.value === '' ? '' : Number(e.target.value))}
        style={{ width:'100%', padding:'8px 10px', borderRadius:8, border:`2px solid ${isEmpty?'#fbbf24':'#10b981'}`, fontFamily:"'DM Mono',monospace", fontSize:13, fontWeight:700, background:isEmpty?'#fffbeb':'#f0fdf4', color:isEmpty?'#92400e':'#065f46', outline:'none', textAlign:'right', boxSizing:'border-box' }} />
      {!isEmpty && <div style={{ fontSize:10, color:'#10b981', marginTop:3, textAlign:'right' }}>ADR ≈ {Math.round(Number(value)/0.75/1000)}K원</div>}
    </div>
  )
}
...
export default function Dashboard() {
  ...
  const simResults = useMemo(() => {
    ...
    return DATE_TYPES.map(dt => {
      ...
      return { dt, simRevPAR, targetRevPAR, simADR:Math.round(rev/ws), simOCC:Math.round(ws*100), pct:targetRevPAR>0?Math.round(simRevPAR/targetRevPAR*100):0 }
    })
  ...
  const updRoomAll = (rId, field, value) => setRoomTypes(prev => ({...prev, [propName]:prev[propName].map(r => r.id===rId?{...r,[field]:value}:r)}))

  const inputProgress = PROP_NAMES.map(p => ({ name:p, filled:roomTypes[p].filter(r=>r.targetRevPAR&&r.targetRevPAR>0).length, total:roomTypes[p].length }))
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#f1f5f9', fontFamily:"'Apple SD Gothic Neo','Noto Sans KR',sans-serif" }}>
      ...
      <div style={{ maxWidth:1120, margin:'0 auto', padding:'22px 36px' }}>
        {/* 컨트롤 바 */}
        <div style={{ display:'flex', gap:8, marginBottom:16, alignItems:'center', flexWrap:'wrap' }}>
          ...
          {rooms?.map(r => {
            ...
            return (
              <button key={r.id} onClick={() => setRoomId(r.id)} style={{ padding:'6px 14px', borderRadius:8, border:`1.5px solid ${roomId===r.id?'#6366f1':hasPAR?'#10b98144':'#fbbf2444'}`, background:roomId===r.id?'#eef2ff':hasPAR?'#f0fdf4':'#fffbeb', color:roomId===r.id?'#6366f1':hasPAR?'#065f46':'#92400e', fontWeight:700, fontSize:12, cursor:'pointer', fontFamily:'inherit' }}>
                {r.name}{!hasPAR&&' ✏️'}
              </button>
            )
          })}
          ...
          {DATE_TYPES.map(dt => {
            ...
            return (
              <button key={dt} onClick={() => setDateType(dt)} style={{ padding:'6px 14px', borderRadius:8, border:`1.5px solid ${dateType===dt?c:'#e2e8f0'}`, background:dateType===dt?`${c}15`:'#fff', color:dateType===dt?c:'#64748b', fontWeight:700, fontSize:12, cursor:'pointer', fontFamily:'inherit' }}>
        ...
        <div style={{ display:'flex', gap:6, marginBottom:16 }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ padding:'8px 16px', borderRadius:9, border:`1.5px solid ${tab===t.id?'#6366f1':'#e2e8f0'}`, background:tab===t.id?'#6366f1':'#fff', color:tab===t.id?'#fff':'#64748b', fontWeight:700, fontSize:12, cursor:'pointer', fontFamily:'inherit' }}>
              {t.label}
            </button>
          ))}
        </div>
        ...
        {tab==='revpar' && (
          <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
            <Card>
              ...
              <Head icon="✏️" title="목표 RevPAR 입력 (평일 기준)" sub="입력하면 요금 테이블 전체가 즉시 계산됩니다." />
              <div style={{ display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:20 }}>
                {rooms?.map(r => (
                  <div key={r.id} style={{ border:`1.5px solid ${r.targetRevPAR&&r.targetRevPAR>0?'#10b981':'#fbbf24'}`, borderRadius:12, padding:18, background:r.targetRevPAR&&r.targetRevPAR>0?'#f0fdf4':'#fffbeb' }}>
                    <div style={{ fontSize:14, fontWeight:800, color:'#0f172a', marginBottom:14 }}>{r.name}</div>
                    <RevPARInput label="목표 RevPAR (평일, 원)" value={r.targetRevPAR} onChange={v => setRoomTypes(prev => ({...prev,[propName]:prev[propName].map(x=>x.id===r.id?{...x,targetRevPAR:v}:x)}))} />
                    <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10, marginTop:14 }}>
                      {[{label:'객실 수',field:'rooms',step:1},{label:'최저가',field:'minPrice',step:1000},{label:'최고가',field:'maxPrice',step:1000}].map(f => (
                        <div key={f.field} style={{ gridColumn:f.field==='rooms'?'1/-1':'auto' }}>
                          <div style={{ fontSize:10, color:'#64748b', fontWeight:600, marginBottom:4, textTransform:'uppercase' }}>{f.label}</div>
                          <input type="number" value={r[f.field]} step={f.step} onChange={e => setRoomTypes(prev => ({...prev,[propName]:prev[propName].map(x=>x.id===r.id?{...x,[f.field]:Number(e.target.value)}:x)}))} style={{ width:'100%', padding:'5px 8px', borderRadius:7, border:'1.5px solid #e2e8f0', fontFamily:"'DM Mono',monospace", fontSize:12, background:'#fff', textAlign:'right', outline:'none', boxSizing:'border-box' }} />
                        </div>
                      ))}
                    </div>
                  </div>
            ...
            <Card>
              {!hasRevPAR ? (
                <div style={{ textAlign:'center', padding:'60px 0', color:'#94a3b8' }}>
                  ...
                  <div style={{ fontSize:16, fontWeight:700, color:'#374151' }}>목표 RevPAR를 먼저 입력해주세요</div>
                  <button onClick={()=>setTab('revpar')} style={{ marginTop:16, padding:'10px 24px', borderRadius:9, background:'#6366f1', color:'#fff', border:'none', fontWeight:700, fontSize:13, cursor:'pointer' }}>RevPAR 입력하러 가기 →</button>
                </div>
              ) : (
                <>
                                ...
                                return (
                                  <td key={ob.key} style={{ padding:3 }}>
                                    <div style={{ background:`${c}12`, border:`1.5px solid ${c}33`, borderRadius:9, padding:'8px 5px', textAlign:'center', outline:clamped?`2px dashed ${c}66`:'none' }}>
                                      ...
                                      {clamped && <div style={{ fontSize:9, color:c, marginTop:2 }}>{price<=room?.minPrice?'최저가':'최고가'}</div>}
                                    </div>
                                  </td>
                                )
                              })}
                ...
                {simResults.map(sr => {
                  ...
                  return (
                    <Card key={sr.dt} style={{ borderTop:`4px solid ${dtc}` }}>
                      ...
                      {[['목표 RevPAR',sr.targetRevPAR],['예상 RevPAR',sr.simRevPAR],['예상 ADR',sr.simADR],['예상 OCC',sr.simOCC+'%']].map(([l,v],i)=>(
                        <div key={i} style={{ display:'flex', justifyContent:'space-between', fontSize:12, color:'#64748b', marginBottom:6 }}>
                          <span>{l}</span><Mono size={12} color={i===1?c:undefined}>{typeof v==='number'?v.toLocaleString()+'원':v}</Mono>
                        </div>
                      ))}
            ...
            <Card style={{ background:'#0f172a', display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center' }}>
              {!hasRevPAR ? (
                ...
                <>
                  ...
                  <div style={{ fontSize:58, fontWeight:800, color:'#f8fafc', fontFamily:"'DM Mono',monospace", letterSpacing:-2 }}>{fmtW(livePrice)}</div>
        ...
        {/* 페이스 분석 */}
        {tab==='pace' && (() => {
          const distData = PACE_BANDS.map(band => { const row={band}; DATE_TYPES.forEach(dt=>{row[dt]=PACE_DISTRIBUTION[propName]?.[dt]?.[band]??0}); return row })
          const allPropData = PROP_NAMES.map(p => ({ name:p.replace(/점$/,''), fullName:p, 단기:((PACE_DISTRIBUTION[p]?.[dateType]?.['D-0~3']??0)+(PACE_DISTRIBUTION[p]?.[dateType]?.['D-4~7']??0)), 중기:((PACE_DISTRIBUTION[p]?.[dateType]?.['D-8~14']??0)+(PACE_DISTRIBUTION[p]?.[dateType]?.['D-15~30']??0)), 장기:((PACE_DISTRIBUTION[p]?.[dateType]?.['D-31~60']??0)+(PACE_DISTRIBUTION[p]?.[dateType]?.['D-61~90']??0)+(PACE_DISTRIBUTION[p]?.[dateType]?.['D-90+']??0)) })).sort((a,b)=>b.단기-a.단기)
          return (
            <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
              <InsightBar propName={propName} />
              <Card>
                <Head icon="📊" title={`${propName} — 리드타임별 예약 점유율`} sub="전체 예약을 100%로 봤을 때, 각 리드타임 구간에서 몇 %가 예약되는지" />
                ...
                <div style={{ display:'flex', height:52, borderRadius:10, overflow:'hidden', gap:2, marginBottom:10 }}>
                  {PACE_BANDS.map((band,i) => { const val=PACE_DISTRIBUTION[propName]?.[dateType]?.[band]??0; if(val<0.5) return null; return (<div key={band} style={{ width:`${val}%`, background:PACE_COLORS[i], display:'flex', alignItems:'center', justifyContent:'center', flexDirection:'column', gap:1, transition:'width 0.4s ease' }}>{val>=6&&<><span style={{fontSize:11,fontWeight:800,color:'#fff'}}>{val}%</span><span style={{fontSize:9,color:'rgba(255,255,255,0.75)'}}>{band}</span></>}</div>) })}
                </div>
                <div style={{ display:'flex', gap:10, marginBottom:20, flexWrap:'wrap' }}>
                  {PACE_BANDS.map((band,i)=>{ const val=PACE_DISTRIBUTION[propName]?.[dateType]?.[band]??0; return (<span key={band} style={{fontSize:11,display:'flex',alignItems:'center',gap:4}}><span style={{width:10,height:10,borderRadius:3,background:PACE_COLORS[i],display:'inline-block'}}/><span style={{color:'#374151',fontWeight:600}}>{band}</span><span style={{color:'#94a3b8'}}>{val}%</span></span>) })}
                </div>
              ...
              <Card>
                ...
                <ResponsiveContainer width="100%" height={260}>
                  <BarChart data={allPropData} layout="vertical" barCategoryGap="20%">
                    ...
                    <Bar dataKey="단기" stackId="s" fill="#ef4444">{allPropData.map((d,i)=><Cell key={i} fill={d.fullName===propName?'#ef4444':'#ef444466'} />)}</Bar>
                    <Bar dataKey="중기" stackId="s" fill="#0ea5e9">{allPropData.map((d,i)=><Cell key={i} fill={d.fullName===propName?'#0ea5e9':'#0ea5e966'} />)}</Bar>
                    <Bar dataKey="장기" stackId="s" fill="#6366f1" radius={[0,4,4,0]}>{allPropData.map((d,i)=><Cell key={i} fill={d.fullName===propName?'#6366f1':'#6366f166'} />)}</Bar>
                  </BarChart>
                </ResponsiveContainer>
              </Card>
            </div>
        ...
        <div style={{ marginTop:20, background:'#0f172a', borderRadius:12, padding:'18px 24px', display:'grid', gridTemplateColumns:'repeat(3,1fr)', gap:20 }}>
          {[{t:'데이터 기반 설계',ls:['3년 104만건 실거래 데이터 분석','지점별 날짜유형 ADR 배율 자동 반영','리드타임별 목표 OCC = 실제 예약 누적 패턴']},{t:'요금 산출 공식',ls:['요금 = 기준ADR × 배율 (최저~최고가 클램핑)','기준ADR = 목표RevPAR ÷ 0.75 × 날짜유형배율','OCC↑ = 배율↑ / 리드타임가깝고 OCC↓ = 배율↓']},{t:'88% OCC 전략',ls:['목표는 88% 달성 + ADR 극대화','빠름 → 올려서 ADR 높이기','느림 → 내려서 88% 채우기','88%+ → 최고가 유지 (보너스 판매)']}].map((s,i)=>(
            <div key={i}>
